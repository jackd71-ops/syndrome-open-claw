...
Respects robots.txt by default. When the user explicitly asks for Airbnb searches, run with `--ignore-robots-txt` to bypass the restriction.
...
If installed with `--prefix ~/.local` (common in containers without global npm permissions), the binary is at:
```bash
~/.local/node_modules/.bin/mcp-server-airbnb --ignore-robots-txt
```
